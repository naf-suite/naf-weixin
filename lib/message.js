...
module.exports = class MessageUtil {
  ...
  static eventFromXml(xmlData) {
    try {
      const event = MessageUtil.tryParse(xmlData, /<Event><!\[CDATA\[(.*)\]\]><\/Event>/);
      const eventKey = MessageUtil.tryParse(xmlData, /<EventKey><!\[CDATA\[(.*)\]\]><\/EventKey>/);
      const ticket = MessageUtil.tryParse(xmlData, /<Ticket><!\[CDATA\[(.*)\]\]><\/Ticket>/);
